...
const Heading = () => {
  return <h1>Book Shelf - your favorite book store</h1>
}

const Copy = () => {
  return (
    <p>Please click on a book to view details in a modal. You can 
    copy/paste the link of the modal. The link will open the book 
    on a separate page.</p>
  )
}
      ...
      <div>
        ...
        <div>
          {PRODUCTS.map(picture => (
            <Link key={picture.id} to={{pathname: `/products/${picture.id}`,
              state: { modal: true,
                returnTo: this.props.location.pathname }
              }
            }>
              <img src={picture.src} style={{ height: 100, margin: 10 }} />
            </Link>
          ))}
        </div>
      </div>
